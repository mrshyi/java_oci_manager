...
# Resolve current Debian bookworm security packages at build time.
...
RUN set -eux; \
    ...
    printf '%s  %s\n' "${expected_sha256}" /tmp/rbot.tar.gz > /tmp/rbot.sha256; \
    sha256sum --check --strict /tmp/rbot.sha256; \
    ...
    rm /tmp/rbot.tar.gz /tmp/rbot.sha256
